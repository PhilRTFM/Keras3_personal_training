# --- Refactored msleep demo pipeline ------------------------------------------
...
# === Data Prep ===
prepare_msleep_data_msleep <- function(prop = 0.7, seed = 123L, verbose = FALSE) {
  ...
  # Retenir uniquement les colonnes utiles
  keep_num <- c("sleep_total", "sleep_rem", "sleep_cycle", "awake", "brainwt", "bodywt")
  keep_cat <- c("vore", "order", "conservation")
  keep_all <- c(keep_num, keep_cat)
  
  dat <- dat[, keep_all, drop = FALSE]
  
  preprocess_data(data = dat, target = "vore", prop = prop, seed = seed, verbose = verbose)
}
...
visualize_raw_data <- function(X_raw, y_fac, verbose = FALSE) {
  ...
  p_mds <- mds_plot(
    ...
    title = "msleep — MDS (brut)", verbose = verbose
  )
...
train_mlp_classifier <- function(X_tr, y_tr_int, X_te, y_te_fac, class_levels, verbose = FALSE) {
  ...
  p_cm <- viz_confusion_panel_from_predictions(
    ...
    title = "msleep — Confusion matrix (MLP, test)", digits = 2L,
...
train_autoencoder <- function(X_tr, X_te, y_tr_fac, y_te_fac, class_levels, X_raw, y_fac, verbose = FALSE) {
  ...
  p_cm <- viz_confusion_panel_from_predictions(
    y_true = y_te_fac, y_pred = y_pred, labels = class_levels,
    title = "msleep — Confusion matrix (AE latent-centroids, test)", digits = 2L,
    show_bottomright = FALSE, verbose = verbose
  )
  ...
  p_mds <- mds_plot(
    ...
    labels = base::rownames(X_raw), group_factor = y_fac,
    title = "msleep — MDS (reconstruit par AE)", verbose = verbose
  )
  
  base::list(
    history = hist, confusion_plot = p_cm,
    pca = p_pca, mds = p_mds,
...
# === Main wrapper ===
run_msleep_demo <- function(seed = 123L, verbose = FALSE, auto_save = TRUE) {
  base::set.seed(seed)
  
  pp <- prepare_msleep_data(prop = 0.7, seed = seed, verbose = verbose)
...
if (base::sys.nframe() == 0L) {
  run_msleep_demo(seed = 123L, verbose = TRUE, auto_save = TRUE)
}
